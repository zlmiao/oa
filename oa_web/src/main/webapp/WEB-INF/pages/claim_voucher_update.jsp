        ...
        <div class="admin-form theme-primary mw1000 center-block" style="padding-bottom: 175px;">
            <div class="panel heading-border">
                <form:form id="admin-form" name="addForm" action="/claim_voucher/update" modelAttribute="info">
                    <form:hidden path="claimVoucher.id"/>                                   <%--修改一定要传递报销单编号，所以隐藏域方式传递--%>
                    <div class="panel-body bg-light">
                        ...
                        <div class="section row" id="items">
                            <c:forEach items="${info.items}" varStatus="sta">                                   <%--页面上显示若干个条目,  foreach把整个div包裹起来，代表没迭代集合一次，生成这么一行--%>
                            <div>
                                <div class="col-md-3">
                                    <label for="items[${sta.index}].item" class="field prepend-icon">
                                        <form:hidden path="items[${sta.index}].id"/>
                                        <form:hidden path="items[${sta.index}].claimVoucherId"/>        <%--每迭代一次还要加两个隐藏域传递值--%>
